...
-----------------------------  InitializeScheduler  ---------------------------------
  ...
  function InitializeScheduler ()
    ...
    --
      ...
      -- print ("Initializing Idle Process...\n")
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method TurnIntoZombie (p: ptr to ProcessControlBlock)
          ...
          for i=0 to MAX_NUMBER_OF_PROCESSES-1
            -- find all children who are zombies and free them
            if processTable[i].parentsPid == p.pid && processTable[i].status == ZOMBIE
              processTable[i].status = FREE
              freeList.AddToEnd(&processTable[i])
          ...
          if parentPCB == null
            ...
            freeList.AddToEnd(p)
            aProcessBecameFree.Signal(&processManagerLock)
          elseIf parentPCB.status == ZOMBIE
            p.status = FREE
            freeList.AddToEnd(p)
            aProcessBecameFree.Signal(&processManagerLock)
      ...
      method WaitForZombie (proc: ptr to ProcessControlBlock) returns int
          ...
          -- Wait on the condition variable and check if proc is ZOMBIE
          while proc.status != ZOMBIE
              aProcessDied.Wait(&processManagerLock)
          endWhile

          -- Get the procs exit status
          exitStatus = proc.exitStatus

          -- Change status to FREE etc
          proc.status = FREE
...
-----------------------------  ProcessFinish  --------------------------
  ...
  function ProcessFinish (exitStatus: int)
    ...
    --
      ...
      oldInterruptStatus = SetInterruptsTo(oldInterruptStatus)

      -- Return all page frames
...
-----------------------------  InitFirstProcess  --------------------------
  ...
  function InitFirstProcess()
      var thrPtr : ptr to Thread
      print("Loading initial program...\n")
      thrPtr = threadManager.GetANewThread()
      (*thrPtr).Init("UserProgram")
      (*thrPtr).Fork(StartUserProcess, "TestProgram4" asInteger)
    endFunction

-----------------------------  StartUserProcess  --------------------------

  function StartUserProcess(arg: int)
      var PCBPtr: ptr to ProcessControlBlock
          fh: ptr to OpenFile
          initUserPC: int
      ...
      -- fh = fileManager.Open("TestProgram1")
      -- load into virtual address space
      fh = fileManager.Open(arg asPtrTo array of char)
      initUserPC = fh.LoadExecutable(&(PCBPtr.addrSpace))
      -- close file handler
      fileManager.Close(fh)

      -- user level stack value
      initUserStackTop = PCBPtr.addrSpace.numberOfPages * PAGE_SIZE

      -- reset system stack top pointer
      initSystemStackTop = (&currentThread.systemStack[SYSTEM_STACK_SIZE-1]) asInteger

      -- return thread to user-lever thread
      -- disabling interrupts
      oldInterruptStatus = SetInterruptsTo(DISABLED)

      -- initialize page table registers for virual addrspace
      PCBPtr.addrSpace.SetToThisPageTable()

      -- Set isUserThread in current Thread
...
-----------------------------  AddrSpace  ---------------------------------
  ...
  behavior AddrSpace
      ...
      method CopyBytesFromVirtual (kernelAddr, virtAddr, numBytes: int)
                    returns int
        --
        -- This method copies data from a user virtual address space
      ...
      method CopyBytesToVirtual (virtAddr, kernelAddr, numBytes: int)
                    returns int
        --
        -- This method copies data from the kernel address space to
      ...
      method GetStringFromVirtual (kernelAddr: String, virtAddr, maxSize: int) returns int
        ...
        -- a pointer to an "array of char" in the kernel code.  This method
...
-----------------------------  Handle_Sys_Exit  ---------------------------------
  ...
  function Handle_Sys_Exit (returnStatus: int)
      ...
      -- printIntVar("return status", returnStatus)
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  function Handle_Sys_Fork () returns int
	  ...
	  for i = 0 to MAX_FILES_PER_PROCESS - 1
		  ...
		  newPCB.fileDescriptor[i] = currentThread.myProcess.fileDescriptor[i]
		endIf
	  endFor
      ...
      -- First get as many new frames as there are in the parent process
      numParentFrames = currentThread.myProcess.addrSpace.numberOfPages
      frameManager.GetNewFrames(&(newPCB.addrSpace), numParentFrames)
      -- Run through each frame and copy
      for i = 0 to numParentFrames - 1
        currFrameAddr = currentThread.myProcess.addrSpace.ExtractFrameAddr(i)
        childFrameAddr = newThrd.myProcess.addrSpace.ExtractFrameAddr(i)
        MemoryCopy(childFrameAddr, currFrameAddr, PAGE_SIZE)

        -- Set writable bits
        if currentThread.myProcess.addrSpace.IsWritable(i)
          newThrd.myProcess.addrSpace.SetWritable(i)
        else
          newThrd.myProcess.addrSpace.ClearWritable(i)
        endIf
      endFor
...
-----------------------------  ResumeChildAfterFork  ---------------------------------
  function ResumeChildAfterFork (oldUserPC: int)
      ...
      -- Initialize value for system stack and user stack
      sysStackTop = (& (currentThread.systemStack[SYSTEM_STACK_SIZE-1])) asInteger
      userStackTop = currentThread.userRegs[14]

      -- Jump into the user-level process
      BecomeUserThread( userStackTop, oldUserPC, sysStackTop)
    endFunction
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  function Handle_Sys_Join (processID: int) returns int
      ...
      for i=0 to MAX_NUMBER_OF_PROCESSES-1
         if processManager.processTable[i].pid == processID                                  
               && processManager.processTable[i].parentsPid == currentThread.myProcess.pid   
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      ...
      -- copying filename from virtual address space
      rValue = currentThread.myProcess.addrSpace.GetStringFromVirtual(
                     &kfilename, 
                     filename asInteger, 
                     MAX_STRING_SIZE)
      if rValue < 0
      ...
      -- load into virtual address space
      initUserPC = fh.LoadExecutable(&(newAddrSpace))
      -- close file handler
      fileManager.Close(fh)
      if initUserPC < 0
         return -1
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
      ...
      -- Check length of filename does not exceed MAX_STRING_SIZE
      --if (filename arraySize > MAX_STRING_SIZE)
        -- return -1
      --endIf

      -- print("its here!\n")

      -- Locate empty slot in fileDescriptor
      fileDescriptorLocation = -1
      for i = 0 to MAX_FILES_PER_PROCESS - 1
         openFile = currentThread.myProcess.fileDescriptor[i]
         if openFile == null
            fileDescriptorLocation = i
            break
         endIf
      endFor

      -- if no null file descriptor slot, return -1
      if (fileDescriptorLocation == -1)
         return -1
      endIf
      ...
      -- Set fileDescriptor to point to openFile
      currentThread.myProcess.fileDescriptor[fileDescriptorLocation] = openFile

      return fileDescriptorLocation
    endFunction
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
      ...
      -- printIntVar("size in bytes", sizeInBytes)
      var
         virtPage: int
         offset: int
         nextPosInFile: int
      ...
      -- Check if the file is previously opened
      elseIf (currentThread.myProcess.fileDescriptor[fileDesc] == null)
         return -1
      
      -- Check that the reqeusted number of bytes is not negative
      ...
      -- first loop checking that file can fit in virtual address space
      while true
         -- Compute the size of this chunk
         thisChunkSize = PAGE_SIZE - offset

         if nextPosInFile + thisChunkSize > fileSize
           thisChunkSize = fileSize - nextPosInFile
         endIf
      ...
      -- repeat loop, but actually read this time
      virtAddr = buffer asInteger
      virtPage = virtAddr/PAGE_SIZE
      offset = virtAddr % PAGE_SIZE
      copiedSoFar = 0
      ...
      while true
         ...
         dumb = fileManager.SynchRead(openFile, destAddr, nextPosInFile, thisChunkSize)
...
-----------------------------  Handle_Sys_Write  ---------------------------------
  ...
  function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
      ...
      -- set address space
      addrSpace = &(currentThread.myProcess.addrSpace)
      ...
      while true
         ...
         if writtenSoFar + thisChunkSize > sizeInBytes
           thisChunkSize = sizeInBytes - writtenSoFar
         endIf
         ...
         -- Check for errors
         if (virtPage < 0) || ( virtPage > addrSpace.numberOfPages - 1 )
                           || !( addrSpace.IsValid(virtPage) )
                           || !( addrSpace.IsWritable(virtPage) )
           return -1
         endIf

         -- increment Values
         nextPosInFile = nextPosInFile + thisChunkSize
         writtenSoFar = writtenSoFar + thisChunkSize
         virtPage = virtPage + 1
         offset = 0
      ...
      while true
         ...
         -- Writing
         addrSpace.SetReferenced(virtPage)
         sourceAddr = addrSpace.ExtractFrameAddr(virtPage) + offset
         dumb = fileManager.SynchWrite(openFile, sourceAddr, nextPosInFile, thisChunkSize)
...
-----------------------------  Handle_Sys_Seek  ---------------------------------
  ...
  function Handle_Sys_Seek (fileDesc: int, newCurrentPos: int) returns int
      ...
      if ( fileDesc < 0 ) || ( fileDesc > MAX_FILES_PER_PROCESS - 1 ) 
                          ...
                          || ( newCurrentPos < -1 )
                          || ( newCurrentPos > currentThread.myProcess.fileDescriptor[fileDesc].fcb.sizeOfFileInBytes)
         fileManager.fileManagerLock.Unlock()
         return -1
      endIf
      ...
      -- check for -1 i.e. EoF
      if (newCurrentPos == -1)
         newCurrentPos = openFile.fcb.sizeOfFileInBytes
      endIf
...
-----------------------------  Handle_Sys_Close  ---------------------------------
  ...
  function Handle_Sys_Close (fileDesc: int)
    ...
    currentThread.myProcess.fileDescriptor[fileDesc].numberOfUsers = currentThread.myProcess.fileDescriptor[fileDesc].numberOfUsers -1

    if (currentThread.myProcess.fileDescriptor[fileDesc].numberOfUsers == 0)
